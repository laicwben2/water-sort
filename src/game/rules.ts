...
export function tryPour(board: Board, from: number, to: number, capacity = DEFAULT_CAPACITY) {
  const move = calculatePour(board, from, to, capacity)
  return move ? { board: applyMove(board, move), move } : null
}
...
export function undoMove(board: Board, move: Move): Board {
  const next = board.map((tube) => [...tube])
  const removed = next[move.to].splice(-move.amount)
  if (removed.length !== move.amount || removed.some((color) => color !== move.color)) {
    throw new Error('Cannot undo move from current board')
  }
  next[move.from].push(...removed)
  return next
}
