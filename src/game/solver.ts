...
function heuristic(board: Board): number {
  return Math.max(0, segmentCount(board) - new Set(board.flat()).size)
}
...
function reconstructSolution(nodes: SearchNode[], solvedNodeId: number): Move[] {
  ...
  while (nodeId !== undefined) {
    const node: SearchNode = nodes[nodeId]
...
export function solveBoard(board: Board, options: SolverOptions = {}): SolverResult {
  ...
  const start: SearchNode = {
    board: startBoard,
    depth: 0,
    estimate: heuristic(startBoard),
    key: startKey,
  }
  const nodes = [start]
  const queue = new MinHeap()
  const bestDepth = new Map([[startKey, 0]])
  queue.push({ nodeId: 0, priority: start.estimate, depth: 0 })
